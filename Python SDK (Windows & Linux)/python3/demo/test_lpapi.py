...
def draw_image():
    ...
    if api.start_job(label_width, label_height):
        ...
        # 绘制矩形框
        api.draw_image_data(DATA_PNG, margin, margin, label_width - margin * 2, label_height - margin * 2)
        # 提交打印任务，开始打印
        api.commit_job()
    #
    api.close_printer()
...
def print_json():
    printer = printers[0] if len(printers) > 0 else None
    print(printer)
    printerName = printer["name"] if isinstance(printer, dict) else None
    print("--- printerName = " + (printerName if printerName else ""))
    # 
    result = api.print(0x01, printerInfo={
    ...
    if isinstance(result, dict):
        ...
        if "previewData" in result:
            print("---- previewData:", result["previewData"])
            previewData.clear()
            previewData.extend(result["previewData"])
        # 
        if "printData" in result:
            print("---- printData  :", result["printData"])
            printData.clear()
            printData.extend(result["printData"])
...
def print_raw_datas():
    ...
    # 
    rawDatas = []
    if isinstance(printData, list):
        for item in printData:
            if isinstance(item, list):
                rawDatas.append(''.join(item))
            # elif isinstance(item, str):
            #     rawDatas.append(item)
            else:
                rawDatas.append(item)
    if len(rawDatas) > 0:
        for data in rawDatas:
            # hex16 字符串打印测试
            api.print_raw_data(data, printerName, copies=2)
            # 二进制字节流打印测试
            # binary_data = bytes.fromhex(data)
            # api.print_raw_data(binary_data, printerName)
            # base64字符串打印测试
            # base64_data = base64.b64encode(binary_data).decode()
            # api.print_raw_data(base64_data, printerName)
